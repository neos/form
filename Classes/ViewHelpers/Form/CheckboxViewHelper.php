...
class CheckboxViewHelper extends \TYPO3\Fluid\ViewHelpers\Form\AbstractFormFieldViewHelper {
	 ...
	 * @param boolean $multiple Specifies whether this is a multo
	 *
	 * @return string
	 * @api
	 */
	public function render($checked = NULL, $multiple = NULL) {
		...
		if ($this->isObjectAccessorMode()) {
			...
			if ($multiple === TRUE || is_array($propertyValue)) {
				if ($checked === NULL && is_array($propertyValue)) {
					$checked = in_array($valueAttribute, $propertyValue);
				}
				$nameAttribute .= '[]';
			} elseif ($checked === NULL && is_bool($propertyValue)) {
